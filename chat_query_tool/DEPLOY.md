...
Streamlit 多页应用：访问 `http://<host>:8501` 后，左侧侧边栏自动出现两个页面切换链接：

| 链接 | 说明 |
|---|---|
| `app` | 主页：玩家**发出**的聊天审查（是否骂人） |
| `💔 收信审查` | 二号页：玩家**收到**的聊天审查（是否被骂） |

两个页面共用同一个 Streamlit 进程、同一个 8501 端口、同一个 Kyuubi 配额池（`MAX_CONCURRENT_USERS`）—— 这意味着两个页面同时查询时会互相排队。

### 5.2 SQL 设计差异

| 场景 | 主页（发件） | 二号页（收信） |
|---|---|---|
| 战斗内 | `battleserver_chat_talk` 直接 `sender = roleid` | 先在 `battleserver_battle_end.accountid = roleid` 找参与的所有 `battle_id`，再回 `chat_talk` 拉这些局的整局聊天（含本人 + 9 位他人） |
| 战斗外 | `gameserver_chat_talk_v2` `sender = roleid AND zoneid = X` | `gameserver_chat_talk_v2` `target = roleid AND sender != roleid AND zoneid = X` |
| 国家 | `query_country` | 完全复用，玩家自身国家不变 |

**为什么战斗内绕一圈走 battle_end**：玩家整局沉默时 `chat_talk` 里没有他的痕迹，但 `battle_end` 里仍有 `accountid` 记录。直接用 chat_talk self-join 找参与的局会漏掉沉默场。

### 5.3 数据持久化

| 路径 | 内容 |
|---|---|
| 宿主机 `./history` ↔ 容器 `/app/history` | 主页发件历史 |
| 宿主机 `./history_received` ↔ 容器 `/app/history_received` | 二号页收信历史 |

两个 history 目录**互相独立**，不会串味。备份命令：

```bash
tar czf history-backup-$(date +%Y%m%d).tar.gz history/ history_received/
```

### 5.4 AI 风险分析的差异

战斗内整局聊天里，**本人发言不送 AI**（`risk_level = -1` 标记，UI 浅蓝底色显示），只对 `sender != roleid` 的他人发言批量送审。统计的违规率 / 屏蔽混淆矩阵分母也只算他人发言。

复用主页 [prompts.py](prompts.py) 的 `IN_BATTLE_SYSTEM` / `OUT_BATTLE_SYSTEM`（prompt 分析的是消息文本本身，与发件人是谁无关）—— 5 分钟内两页查询互相吃 prompt cache。

### 5.5 服务器更新

代码改动后从开发机推到 git 仓库，服务器侧：

```bash
cd ~/chat_query_tool
git pull
mkdir -p history_received       # 第一次启动二号页前手动建一下宿主机挂载点
docker compose up -d --build    # --build 会把 pages/ 目录拷进新镜像
docker compose logs -f chat-audit
```

容器启动后，浏览器刷新 `http://<host>:8501`，左侧侧边栏出现两个链接即代表多页加载成功。
...
| 文件 | 作用 |
|---|---|
| [app.py](app.py) | Streamlit 主页（发件审查）入口、流程编排 |
| [pages/2_💔_收信审查.py](pages/2_💔_收信审查.py) | 二号页：收信审查入口、流程编排 |
| [ui_helpers.py](ui_helpers.py) | 主页和二号页共用的纯函数（校验 / 高亮 / 总览表 / ...） |
| [db.py](db.py) | Kyuubi 查询封装、engine 切换、并发限流；含 `query_user_chats` / `query_received_chats` |
| [analyzer.py](analyzer.py) | Claude 调用、批量分类、prompt cache |
| [prompts.py](prompts.py) | 分类器 system prompt（两页共用） |
| [history.py](history.py) | 主页发件查询持久化（`history/`） |
| [history_received.py](history_received.py) | 二号页收信查询持久化（`history_received/`） |
| [received_helpers.py](received_helpers.py) | 二号页特有：本人 vs 他人区分、收信场景统计、Excel 导出 |
| [Dockerfile](Dockerfile) | 镜像构建 |
| [docker-compose.yaml](docker-compose.yaml) | 容器编排（含两个 history volume） |
